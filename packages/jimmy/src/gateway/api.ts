...
export interface ApiContext {
   ...
   * ~/.openbanto/config.yaml when the same code path is also going to call
...
Handle this as a priority request from a colleague.`;
    ...
    if (method === "POST" && pathname === "/api/onboarding") {
      ...
      const effectiveName = portalName || "Banto";
    ...
    if (rateLimit.limited) {
      ...
      if (currentSession.engine === "claude" && strategy === "fallback") {
        ...
        if (fallbackEngine) {
          ...
          context.emit("session:completed", {
            sessionId: currentSession.id,
            employee: currentSession.employee || config.portal?.portalName || "Banto",
