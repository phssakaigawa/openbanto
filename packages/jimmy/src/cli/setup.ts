...
export async function runSetup(opts?: { force?: boolean }): Promise<void> {
  ...
  // Derive default COO name from instance name if set, otherwise "Banto"
  ...
  if (!fs.existsSync(CONFIG_PATH)) {
    ...
    if (chosenName !== "Banto") {
      // Template already has `portalName: Banto` under portal. Substitute.
      // Also handle the legacy `portal: {}` form for back-compat.
      if (source.includes("portalName: Banto")) {
        source = source.replace("portalName: Banto", `portalName: "${chosenName}"`);
      } else {
  ...
  // ~/.openbanto/migrations/. Existing applied migrations stay (skip-if-exists),
