...
- `~/.openbanto/IDENTITY.md` — 名前・雰囲気・絵文字・由来。あなたが「誰であるか」
- `~/.openbanto/SOUL.md` — トーン・意見の出し方・ユーモア・境界線。あなたが「どう振る舞うか」
- `~/.openbanto/MEMORY.md` — 長期記憶（事実・好み・決定）
...
`~/.openbanto/BOOTSTRAP.md` が存在する場合、**他の何よりも先に**そのファイルを読み、書かれた手順を最後まで実行してください。完了後は BOOTSTRAP.md 自身を `rm` で削除します。BOOTSTRAP.md が存在しない場合はオンボーディング済みなので、通常運用に入ります。
...
スキルは `~/.openbanto/skills/<skill-name>/SKILL.md` に格納されたMarkdownプレイブックです。コードではなく、ステップバイステップで従う指示書です。
...
- **部門**は `~/.openbanto/org/<department-name>/` 以下のディレクトリ
- 各部門には `department.yaml`（メタデータ）と `board.json`（タスクボード）がある
- **従業員**は `~/.openbanto/org/<department>/<employee-name>.yaml` のYAMLペルソナファイル
...
定期実行ジョブは `~/.openbanto/cron/jobs.json` で定義されます。ゲートウェイがこのファイルを監視し、変更時に自動リロードします。
...
- 実行ログは `~/.openbanto/cron/runs/` に保存される
...
`~/.openbanto/` 内の任意のファイルを編集できます。ゲートウェイが変更を監視し反応します：
...
ゲートウェイのアーキテクチャ、コネクタプロトコル、エンジン能力、設計判断について深く理解するには `~/.openbanto/docs/` を読んでください。本ファイル以上の文脈が必要なときに参照します。

---

## スラッシュコマンド

ユーザーはチャットでスラッシュコマンドを入力できます。各コマンドには `~/.openbanto/skills/<command>/SKILL.md` に処理方法を教えるスキルプレイブックがあります。
