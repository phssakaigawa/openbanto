// WhatsApp support is an OPTIONAL plugin. `@whiskeysockets/baileys` (GPL-3.0) is
// NOT a core dependency of OpenBanto and is NOT bundled or redistributed. To
// enable WhatsApp, install it yourself:  npm i @whiskeysockets/baileys
// It is loaded lazily via dynamic import below so the core stays MIT-clean and
...
async function loadBaileys(): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      "WhatsApp connector requires the optional '@whiskeysockets/baileys' package (GPL-3.0), " +
        "which is not bundled with OpenBanto. Enable WhatsApp with: npm i @whiskeysockets/baileys",
    );
  }
  const ns: any = mod;
  const def: any = ns.default ?? ns;
  // Handle ESM/CJS interop for the default export (makeWASocket).
  makeWASocket = (def && def.default) ? def.default : def;
  // Named exports live on the namespace (ESM) or the default (CJS).
  const named: any = ns.useMultiFileAuthState ? ns : def;
  Browsers = named.Browsers;
  DisconnectReason = named.DisconnectReason;
  fetchLatestWaWebVersion = named.fetchLatestWaWebVersion;
  useMultiFileAuthState = named.useMultiFileAuthState;
  downloadMediaMessage = named.downloadMediaMessage;
}
import type {
  Connector,
