...
program
  .command("setup")
  .description("OpenBantoを初期化し依存関係をインストールする")
...
program
  .command("create <name>")
  .description("新しいOpenBantoインスタンスを作成する")
...
program
  .command("list")
  .description("すべてのOpenBantoインスタンスを一覧表示")
  .action(async () => {
...
program
  .command("remove <name>")
  .description("OpenBantoインスタンスをレジストリから除外する")
...
program
  .command("nuke [name]")
  .description("OpenBantoインスタンスと全データを完全に削除する")
...
// 切り離しジョブ（ryoko job run / list — 完了時にセッションを自動起床）
{
  ...
  jobCmd
    ...
    .option("--log <path>", "ログファイルのパス（省略時は ~/.openbanto/jobs/logs/<id>.log）")
