...
export function buildContext(opts: {
  ...
  const portalName = opts.portalName || opts.config?.portal?.portalName || "Banto";
...
export function buildDetachedJobsContext(sessionId?: string, jobsDir?: string): string | null {
  ...
  lines.push(`Read each log, complete or recover the deferred work, then delete the job's state file under \`~/.openbanto/jobs/\` so this list clears.`);
  return lines.join("\n");
}
...
function buildEvolutionContext(portalName: string, config?: JinnConfig): string {
  ...
  if (isNew) {
    ...
    if (canvasHintApplies) {
      ...
      lines.push(
        `- The user just asked something that benefits from seeing live session state (e.g. "what is Banto doing right now?", "I want a dashboard of your work", "is there a way to see all the agents?")`,
      );
      lines.push(
