...
`docs/design/http-engine-skeleton.md` の申し送りを**組込み実装**として取り込んだもの。任意名のエンジンを **1 実装（openai）で複数**定義でき（`engines.openai-1`/`engines.openai-2` …）、各々 baseUrl/apiKey/model を持つ。CLI ではなく HTTP（`/v1/chat/completions` `stream:true`）で会話し、SSE を `onStream` に転送、最終テキストを `EngineResult.result` に集約。**tool-call/MCP はスコープ外**（`mcpConfigPath` 無視・TODO）。
...
| engine map 構築 | `gateway/server.ts`（builtin + `module` **または** `impl` を持つ config engine を列挙、`cfg.name`＝config キーを注入） | impl エンジンの `name` は config キー（openai-1/openai-2）。列挙条件に impl を落とすと生成されない |
