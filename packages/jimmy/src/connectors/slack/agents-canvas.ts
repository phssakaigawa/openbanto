/**
 * Agents View for Slack — a self-updating Canvas that mirrors OpenBanto's
 * current sessions, inspired by Claude Code v2.1.139's `claude agents`.
 *
 * Data source: OpenBanto's own session registry (SQLite via `listSessions()`).
 * NOT `~/.claude/daemon/roster.json` — that file is populated only by
 * `claude --bg` background sessions, and OpenBanto currently spawns one-shot
...
export interface AgentsCanvasConfig {
  ...
  channelId?: string;
  /** Display title. Defaults to "Banto Agents View". */
  title?: string;
...
const DEFAULT_TITLE = "Banto Agents View";
